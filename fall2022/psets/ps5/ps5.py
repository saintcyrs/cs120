...
def bfs_2_coloring(G, precolored_nodes=None):
    ...
    # While visited < total number of nodes, run BFS on each graph component
    while len(visited) < G.N:
        lone = nodes - visited
        frontier = [list(lone)[0]]
        for edge in frontier:
            if G.colors[edge] == None:
                G.colors[edge] = 0
            for connected in G.edges[edge]:
                if G.colors[connected] == G.colors[edge]:
                    G.reset_colors()
                    return None
                if connected not in visited:
                    if G.colors[edge] == 0:
                        G.colors[connected] = 1
                    if G.colors[edge] == 1:
                        G.colors[connected] = 0
...
def is_independent_set(G, subset):
    for node in subset:
        for connected in G.edges[node]:
            if connected in subset:
                return False
        return True
...
def iset_bfs_3_coloring(G):
    vertices = G.N
    size = int(vertices/3) + 1
    # Go through all iterations of various sizes
    for num in range(size):
        # Create subsets based on nums
        for subset in combinations((range(vertices)), num + 1):
            subset = list(subset)
